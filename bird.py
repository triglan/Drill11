...
PIXEL_PER_METER = (10.0 / 0.3)  # 10 pixel 30cm, bird에 넣는게 좋을 듯
...
class Run:
    ...
    @staticmethod
    def draw(bird):
        if bird.frame >= 0 and bird.frame < 5:
            if bird.dir == 1:
                bird.image.clip_draw(int(bird.frame) // 5 * 182, 2 * 168, 183, 168, bird.x, bird.y)
            if bird.dir == -1:
                bird.image.clip_draw(int(bird.frame) // 5 * 182, 2 * 168, 183, 168, bird.x, bird.y)
        if bird.frame >= 5 and bird.frame < 10:
            bird.image.clip_draw(int(bird.frame)//5 * 182, 1 * 168, 183, 168, bird.x, bird.y)
        if bird.frame >= 10 and bird.frame < 14:
            bird.image.clip_draw((int(bird.frame) - 10) * 182, 0 * 168, 183, 168, bird.x, bird.y)
        print(f'frame : {bird.frame}')
...
class StateMachine:
    def __init__(self, bird):
        ...
        self.cur_state = Run
        self.transitions = {
            Idle: {right_down: Run, left_down: Run, left_up: Run, right_up: Run, time_out: Sleep, space_down: Run},
